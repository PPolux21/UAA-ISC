...
int main(int argc,char **args) {
    ...
    for(int i=0;i<PERSON;i++)
	{
		if(i%2==0)
		{		
			vector[i]=CPersonaje("pacman_mini.png",rand()%ANCHO,rand()%ALTO,rand()%1500,rand()%1500);
		}else{
			vector[i]=CPersonaje("pacman_mini_2.png",rand()%ANCHO,rand()%ALTO,rand()%1500,rand()%1500);
		}
...
//    al_draw_bitmap(per1.imgPersonaje, per1.xPer, per1.yPer, 0);
