...
CEnemigo evil(200,200,TAM*4,TAM,0,DERECHA,true);
...
int main(int argc,char **args) {
    ...
    imgPersonaje = al_load_bitmap("steve.png");
    imgPersonaje2 = al_load_bitmap("steve_2.png");
    xPer = TAM;
    yPer = TAM;
    vXPer = 200;
    vYPer = 200;
...
void pintar(int i){
    ...
    sprintf(cadena, "Score: %d", pts*10);
    al_draw_text(font, al_map_rgb(255, 255, 255), 650, 0, 0, cadena);

	sprintf(cadena, "Direccion: %d", evil.direction);
    ...
    al_draw_bitmap(imgPersonaje, evil.xPer, evil.yPer, 0);
    al_draw_circle(evil.xPer+3, evil.yPer+3, 1,al_map_rgba_f(1, 1, 1, 1), 0);
	al_draw_circle(evil.xPer+22, evil.yPer+3, 1,al_map_rgba_f(1, 1, 1, 1), 0);
	al_draw_circle(evil.xPer+22, evil.yPer+22, 1,al_map_rgba_f(1, 1, 1, 1), 0);
	al_draw_circle(evil.xPer+3, evil.yPer+22, 1,al_map_rgba_f(1, 1, 1, 1), 0);
...
void fisica(float dt){
	...
	if(direccion==ABAJO){
		jMat=(xPer+4)/TAM;// x
		iMat=yPer/TAM;// y
		hMat=(xPer+21)/TAM;	// x a la derecha
	...
	if(direccion==ARRIBA){
		...
		yPer -= vYPer * dt;
//		if((mat[iMat-1][jMat]!=1 && mat[iMat-1][hMat]!=1)){
//			yPer -= vYPer * dt;	
//		}
